    ...
    try {
        ...
        for (const giveawayUrl of pendingGiveaways) {
            try {
                ...
                var content = fs.readFileSync("./pending-giveaway.txt", "utf8").toString().replace(giveawayUrl, "");
                fs.writeFileSync("./pending-giveaway.txt", content);
                fs.appendFileSync("./done-giveaway.txt", giveawayUrl + "\n")
            } catch (error) {
                console.log(error);
            }
        }
